...
The chart of accounts is versioned and governed. The balance-bearing ledger-account identity is stable; its accounting classification lives in an immutable mapping for each chart version. Each mapping declares account code, the account's immutable currency, account class, normal balance, currency policy, product/control-account role and permitted posting directions. Its code identity is `(book, chart version, account code, account currency)`, so a valid `1000/NGN` account and `1000/USD` account remain distinct; a composite foreign key prevents a mapping currency from differing from the ledger account's currency. At most one chart version is `ACTIVE` for a book, activation and retirement facts are immutable, and every journal pins the governed chart version used for all of its posting lines.

Every newly created chart starts in `DRAFT`. Mapping insert, update and delete are permitted only in `DRAFT`; activation locks governance and rejects the candidate unless every open ledger account in the book has exactly one currency-consistent mapping. Chart creation/activation, mapping changes and pre-activation open-account changes all advance a stable book-level governance revision. A snapshot-isolated operation that waited behind any of those changes must therefore serialize rather than validate a stale account or mapping universe, including when the candidate chart did not yet exist in the other transaction's snapshot. The complete mapping set is frozen after activation.

Operational rotation is one governed database operation, not two lifecycle updates. It locks the current and candidate chart rows in canonical UUID order, then the stable book row; revalidates same-book ownership, current `ACTIVE` and forward candidate `DRAFT` state, complete currency-consistent mappings and effective bounds; and retires/activates both rows atomically. Mapping mutation and posting/direct-journal guards follow the same chart-before-book order. Chart validity is half-open `[activated_at, retired_at)`: a rotation boundary cannot predate the current activation, lie in the future, or invalidate an existing current-chart journal at or after that boundary. `funds_app` cannot update chart lifecycle rows or execute the owner-only operation. Initial empty-book bootstrap and exceptional repair remain trusted migration-owner actions; after activation, controlled operators use the governed operation. Rotation creates and validates a complete new mapping set before activation; it does not update old mappings or reclassify historical journals.

The PoC deliberately rejects creating or reopening an open account after a chart is active because the governed atomic workflow that would extend an active classification set is not implemented; live account onboarding/reopening is a subsequent delivery slice. Branch, product, channel, provider and cost-centre are posting dimensions, not substitutes for legal accounts. The PoC implements the dimensions required for its examples but does not implement group consolidation or regulatory chart mapping.
...
Accounting periods transition `OPEN -> CLOSING -> CLOSED` under maker-checker control. `funds-core` accepts ordinary postings only into an open period owned by the selected book. Both the service and independent commit-time database guards derive the booking date in the book timezone, require booking and value dates to belong to the selected period, pin the current accounting-policy and effective chart versions, and lock the governing rows before financial writes. Period close takes the conflicting governance lock, so it cannot overtake an in-flight posting; after the transition begins, no new ordinary posting can enter that period. End-of-day is a restartable set of independently identified jobs: expire eligible holds, ingest cutoff evidence, calculate configured accruals, run subledger/GL and nostro proofs, snapshot signed control totals, and close the business date. A failed job is resumed with the same run ID; it does not repeat a financial effect.
...
The proof reads immutable postings rather than trusting materialised balances. The PoC's independent proof job uses a separate `funds_proof_reader` login with column-level access only to immutable journal/posting facts, governed control mappings and projection totals; it cannot read identifiers, product policy, idempotency results or outbox payloads. The control projection has no history in this slice, so control proof is explicitly current-cutoff-only; historical-cutoff control claims require a versioned projection history. Differences create a P0 incident and block period close. Production extensions include regulatory mapping, consolidation, capital adequacy, liquidity ratios and product profitability.

### 8.12 Monetary range, precision and rounding

Posted amounts use checked signed 64-bit integer minor units. APIs reject values outside configured product and currency maxima before arithmetic. Because exact reversal negates every amount, `Long.MIN_VALUE` is outside the reversible PoC domain even though other signed 64-bit values remain valid. A journal contains 2–256 postings; each posting has at most 32 dimensions and at most 8,192 UTF-8 bytes in its persisted canonical dimension JSON object. Dimension values are strings, matching the typed `Map<String,String>` contract without JSON coercion. Those type and size bounds are enforced by both the service and database so every accepted journal remains exactly reversible within the memory budget. Java uses exact integer operations that throw on overflow. Go uses explicit checked add/subtract helpers; neither language relies on wraparound. Database aggregates use a numeric type large enough to detect an application overflow rather than reproduce it.
...
Every posting and reversal request carries a versioned canonical hash of its typed command. `funds-core` re-derives that hash from the trusted typed fields before opening a transaction and rejects a stale or caller-invented value; the supplied hash is never treated as an attestation. The hash includes every financially relevant field, including the governed chart version and canonical dimension key/value pairs. Both idempotency commands and journal facts persist their hash-scheme version, and fixed golden vectors freeze the bytes represented by each durable scheme label. New facts use the domain-separated typed-command V2 and chart-pinning journal V2 schemes. Migrated V004 bytes are never silently rewritten: the journal remains tagged `V004_V1` and is verified with the exact pre-chart canonical algorithm; its completed opaque command hash is not trusted for equality. Posting replay reconstructs and verifies the immutable journal before deriving a trusted typed V2 posting-command hash. Completed legacy reversal replay similarly reconstructs the persisted reversal header and bounded postings, verifies the V004 journal hash, and derives the typed V2 reversal-command hash without consulting current chart or period governance. A completed same-content replay is therefore resolved before policy or governance rules that may have changed since the original commit, while a command ID with any changed typed field remains a deterministic conflict. Every completed idempotency row is also constrained to its own journal, and its result JSON must exactly cache that journal's ID, sequence and canonical hash. The generic posting API rejects caller-supplied reversal metadata; only the reversal service can use its narrow trusted posting path. Reversal verification dispatches by the persisted journal scheme, so authentic V004 facts remain exactly reversible after upgrade.
...
Application validation is backed by database constraints and deferred constraint triggers that independently enforce governed book/period/policy/chart ownership, complete per-line chart mapping, typed reversible posting/dimension bounds, per-currency zero-sum validation, coherent completed-command linkage/results and immutable finality. A journal linked as a reversal must have transaction type `REVERSAL`, may be linked only once, and at completion must be the exact account/currency/amount/dimension multiset negation of the original. Database roles used by the services cannot disable these guards or update/delete journals and postings.
...
The framework must be measured. A lightweight build is preferred; a heavier framework is acceptable only if ACC-25 passes. The accounting-kernel PoC fixes the Quarkus worker pool at 2–8 platform threads with a queue of 32; overflow is rejected with explicit backpressure before a financial transaction begins. JDBC pools are bounded per replica, request and executor queues are bounded, and concurrency is rejected before it creates unbounded platform or virtual threads. Statements, reconciliation results and journal exports stream in pages; idempotency responses and narrations have byte limits. Local caches use maximum weight and expiry and never hold authoritative balances.
...
PgBouncer or an equivalent pooler protects the server from replica connection multiplication. Each service has a connection budget and acquisition timeout; `funds-core` keeps a small reserved pool for idempotency/status recovery. Before any posting or reversal lock/statement work, the PoC applies transaction-local `lock_timeout = 1s`, `statement_timeout = 3s` and `idle_in_transaction_session_timeout = 5s`. Lock and statement deadline failures map to a typed timeout for explicit caller/client policy; `funds-core` does not internally retry SQLSTATE `55P03` or `57014`. Internal retry orchestration remains limited to the explicitly classified serialization/deadlock cases. Temporary-file bytes, WAL generation, archive backlog, checkpoints, connection wait and table/index growth are acceptance metrics.
