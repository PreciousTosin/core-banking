...
The chart of accounts is versioned and governed. The balance-bearing ledger-account identity is stable; its accounting classification lives in an immutable mapping for each chart version. Each mapping declares account code, account class, normal balance, currency policy, product/control-account role and permitted posting directions. At most one chart version is `ACTIVE` for a book, activation and retirement facts are immutable, and every journal pins the governed chart version used for all of its posting lines. Rotation creates and validates a complete new mapping set before activation; it does not update old mappings or reclassify historical journals. Branch, product, channel, provider and cost-centre are posting dimensions, not substitutes for legal accounts. The PoC implements the dimensions required for its examples but does not implement group consolidation or regulatory chart mapping.
...
Every posting and reversal request carries a versioned canonical hash of its typed command. `funds-core` re-derives that hash from the trusted typed fields before opening a transaction and rejects a stale or caller-invented value; the supplied hash is never treated as an attestation. The hash includes every financially relevant field, including the governed chart version and canonical dimension key/value pairs. A completed same-content replay is resolved from the stored result before applying policy or governance rules that may have changed since the original commit. A command ID with any changed typed field remains a deterministic conflict. The generic posting API rejects caller-supplied reversal metadata; only the reversal service can use its narrow trusted posting path.
...
Application validation is backed by database constraints and deferred constraint triggers that independently enforce governed book/period/policy/chart ownership, complete per-line chart mapping, the reversible posting/dimension envelope, per-currency zero-sum validation and immutable finality. A journal linked as a reversal must have transaction type `REVERSAL`, may be linked only once, and at completion must be the exact account/currency/amount/dimension multiset negation of the original. Database roles used by the services cannot disable these guards or update/delete journals and postings.
