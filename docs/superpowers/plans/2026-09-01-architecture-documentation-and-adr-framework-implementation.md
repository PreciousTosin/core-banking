...
- Every newly introduced non-retrospective ADR starts as `Proposed`. Direct introduction as `Accepted` or `Rejected` is valid only when `Retrospective: Yes` and its path-bound historical evidence verifies successfully, except for the exact one-time ADR-0001 framework-adoption record described and tested in Task 5; that exception is bound to the approved design at commit `0e46650dcb382bf4ddc040e0ec73e98675dff40b` and is not a reusable lifecycle edge.
...
- Permanent repository validation parses both architecture tooling manifests, requires `@mermaid-js/mermaid-cli` to be declared and locked exactly at `11.16.0`, and rejects tracked content below `architecture/tooling/node_modules/` or `architecture/diagrams/generated/`. SVG files elsewhere are allowed only when their governing Markdown explicitly classifies them as an approved derivative and links the source from which they were generated.
...
- `architecture/archive/proposals/README.md`: terminal `implemented`, `rejected`, and `superseded` records move here under the same basename; document status-specific `implementation_status` and `replacement` rules plus mandatory closure evidence and ADR history. `related_plans` is required and reciprocal when present; implementing/implemented delivery requires at least one plan, while rejected/superseded records that never entered planning use literal `None`. A registry pointer changes during the move, while accepted ADR and any plan backlinks continue to use the stable registry anchor.
...
**Interfaces:**
- Consumes: the reserved pre-section document preamble plus all 27 top-level sections of `architecture/modern-core-banking-comprehensive-design-revised.md`.
...
- The unnumbered content before `## 1.` has the one reserved source key `00.document-preamble`. Tokenize it independently into `P01` for the `# Modern Core Banking System` title plus `## Comprehensive Architecture and Single-VPS Proof-of-Concept Design` subtitle, `P02` for the contiguous Status/Version/Date/Base currency/Audience metadata run, and `P03` for the thematic break. Require the exact resolved row `| 00.document-preamble | Document title, status, version, date, currency, and audience preamble | P01; P02; P03 | historical-only | None | None | The source-document identity and revision metadata describe the archived publication itself; no maintained current or proposed destination is appropriate. | resolved |`. Add independent failures for an omitted preamble row, any missing/extra/duplicate preamble token, any other `00.*` key, a non-historical disposition, or any destination other than literal `None`.
...
- A resolved proposal-disposition destination must be an explicit permanent anchor under `architecture/proposals/README.md#<governed-identity>`, never an anchor in the mutable active/archive proposal record. Require exactly one immediately following Markdown pointer, resolve it through the governed registry, and prove it names the identity's sole `architecture/proposals/<basename>.md` or `architecture/archive/proposals/<basename>.md` record. The migration marker remains beside the stable registry anchor, so changing the pointer during proposal archival does not mutate the reviewed inventory or its destination. Add focused failures for an active-file/archive-file destination, a missing/duplicate registry pointer, a stale pointer, a pointer to the wrong basename, and a marker placed in the mutable record instead of beside the registry anchor. Task 4's deliberately unresolved provisional proposal rows may still use their temporary governance anchors until Task 7 creates the records and performs this remap.
...
rows = []
rows.append("| 00.document-preamble | Document title, status, version, date, currency, and audience preamble | P01; P02; P03 | historical-only | None | None | The source-document identity and revision metadata describe the archived publication itself; no maintained current or proposed destination is appropriate. | resolved |")
for section in range(1, 28):
    if section == 8:
...
Inventory the pre-section source-document preamble as the exact `00.document-preamble` row and inventory every material `##`, `###`, and `####` heading within numbered sections 1 through 27, not merely the 27 top-level headings. For the preamble, use only the `P01`, `P02`, and `P03` tokenizer and exact historical-only row defined in Step 1; it is never absorbed into section `01` or omitted because it lies before the first numbered heading. Derive numbered-section source keys from the printed heading number: zero-pad each numeric component (`8.1.1` becomes `08.01.01`); named worked examples use a lowercase slug (`13.08.example-a`). If one heading mixes dispositions, split it into `08.01::01`, `08.01::02`, and later contiguous segment keys, with one row per classified segment. Full source keys must be unique, the reserved key must occur exactly once, and top-level roots `01` through `27` must all be represented.
...
Treat the inventory as the sole source of truth for migration markers. Deduplicate its non-historical block mappings into expected `(path, anchor, source-key)` triples. Proposal triples always name permanent governed-identity anchors in `architecture/proposals/README.md`; resolve each anchor's sole mutable pointer to the exact active/archive same-basename record as an additional validity check, but never substitute that mutable path into the inventory tuple. After masking fenced and inline code, globally scan every governed Markdown file for active full-line marker comments, associate each marker with the explicit HTML anchor whose contiguous marker block immediately follows it, and require multiset equality with the expected triples. Therefore a missing, orphan, extra, duplicate, wrong-file, wrong-anchor, wrong-key, mutable-record proposal marker, or superseded provisional marker is blocking even when all active destinations themselves validate.
...
Add `migration` to `CHECKS` and `VALIDATORS` and implement `validate_migration_inventory(root: Path) -> list[str]` with the exact schema, reserved preamble key/tokenizer/row, source-key grammar, allowed values, top-level and subsection coverage, uniqueness, material-block tokenization and exact-once coverage, non-empty per-row rationale, historical-only rationale, contiguous mixed-segment suffix, exact destination-block mapping, destination-file and anchor existence, proposal registry-pointer resolution, exact source-marker backlink, current-evidence, and unresolved-row rules above. Parse the pre-section title/subtitle/metadata/thematic-break preamble, numbered `##`/`###`/`####` headings from the comprehensive source, plus unnumbered `#### Example A` through `#### Example J` under section 13.8. Require the reserved preamble key exactly once and each derived numbered material heading key to have either one exact inventory key or one or more segment keys with that key plus `::NN`; reject inventory keys that do not map back to the preamble or a source heading. Table parsing must report malformed rows rather than silently skipping them. Emit each unresolved row exactly as `architecture/archive/comprehensive-design-migration-inventory.md: unresolved migration row <SOURCE_KEY>` so interim gates can compare the complete diagnostic set mechanically.
...
Test introduction status independently from later lifecycle edges. A newly introduced ADR with `Retrospective: No` must have `Status: Proposed`; direct `Accepted` and direct `Rejected` introductions fail. A newly introduced ADR with `Retrospective: Yes` may be `Accepted` or `Rejected` only when `Implementation evidence` contains at least one verified historical `changed` or `snapshot` entry whose commit predates the introduction child; missing, nonexistent, future/self-child, path-missing, or mode-invalid evidence fails even when ordinary field validation would allow `None`. For a working-tree endpoint (`head_ref=None`), require the evidence commit to be the base commit or its ancestor, which proves it predates the not-yet-committed child. Add positive direct-Accepted and direct-Rejected retrospective fixtures with verified pre-introduction evidence. Add one narrow positive fixture for the non-retrospective bootstrap exception: exact path `architecture/adr/0001-manage-architecture-as-versioned-code.md`, exact title `# ADR-0001: Manage architecture as versioned code`, `Decision date: 2026-09-01`, the exact framework-plan reciprocal link, and the first implementation-evidence entry `- 0e46650dcb382bf4ddc040e0ec73e98675dff40b changed: docs/superpowers/specs/2026-09-01-architecture-documentation-and-adr-framework-design.md`. Verify that commit/path/mode normally, require that commit to be an ancestor of the introduction child (or of the base for a working-tree endpoint), and require the approved design bytes at that commit to contain the exact Markdown line `**Status:** Approved design`. Mutating any bound field/path/hash/title, applying the exception to another ADR, or introducing ADR-0001 as `Rejected` fails. This is an exact adoption record, not a general ADR-number exemption.
...
- an exact qualifying retrospective ADR introduced as `Accepted` or `Rejected` passes its introduction edge only when its verified historical evidence predates that child; an otherwise identical non-retrospective introduction fails. The exact ADR-0001 framework-adoption fixture above is the only non-retrospective direct-Accepted introduction exception. In every passing case, the child establishes the protected baseline and a rationale mutation in the next commit fails;
...
| `test_introduction_protection_starts_after_qualified_child` | Qualifying `Retrospective: Yes` plus verified pre-introduction evidence at `Accepted/context-v1` -> `Accepted/context-v2` | Qualified introduction edge passes; second edge fails. A companion non-retrospective introduction fails on its first edge. |
...
Use `Status: Accepted`. Every ADR links at least one exact affected arc42 file under `Related architecture sections`, and each linked arc42 file includes the reciprocal ADR ID in `related_adrs`. ADR-0001 is the exact one-time non-retrospective framework-adoption record: use title `# ADR-0001: Manage architecture as versioned code`, `Status: Accepted`, `Retrospective: No`, `Decision date: 2026-09-01`, and the framework governance scope required by the exception test. Its first implementation-evidence entry is exactly `- 0e46650dcb382bf4ddc040e0ec73e98675dff40b changed: docs/superpowers/specs/2026-09-01-architecture-documentation-and-adr-framework-design.md`; verify the commit, path, changed-path claim, ancestry, and approved-design status before writing it. Implementation status is `Partial` until the framework is complete. Under `Related implementation plans`, ADR-0001 contains exactly `[Architecture Documentation and ADR Framework Implementation Plan](../../docs/superpowers/plans/2026-09-01-architecture-documentation-and-adr-framework-implementation.md)`; ADRs without a related plan use `None`. After ADR-0001 exists, insert exactly this single metadata line in this plan's header immediately after `**Spec:**`, without changing any task prose, checkbox, command, ordering, or other semantics:
...
- Produces: permanent `tooling` repository validation that parses both npm manifests, enforces the exact Mermaid CLI version, and rejects tracked dependency/generated-output paths.
...
Also write failing tests for permanent `validate_tooling(root)`: parse JSON rather than searching text; require `package.json` to contain only the declared exact `@mermaid-js/mermaid-cli: 11.16.0` development pin and require `package-lock.json` root metadata plus `packages["node_modules/@mermaid-js/mermaid-cli"].version` to resolve exactly `11.16.0`. Reject missing/malformed manifests, ranges/carets, another direct architecture-tooling dependency, and a lockfile root declaration or resolved package at any other version. In temporary Git repositories, reject every tracked path below `architecture/tooling/node_modules/` and `architecture/diagrams/generated/`, regardless of extension or ignore rules. Do not globally ban SVG: a tracked SVG elsewhere under `architecture/` passes only when governed Markdown contains the exact full-line marker `<!-- approved-architecture-derivative: repository/path.svg source=architecture/diagrams/source.mmd -->`, both paths exist, and that Markdown links the SVG; add positive and orphan/missing-source/wrong-path negative fixtures. SVG outside `architecture/` remains outside this check.
...
Add `diagrams` and `tooling` to `CHECKS` and `VALIDATORS`. Implement the exact filename, first-ten-lines metadata/front-matter, state, abstraction, question, owner, arc42-path, ADR-ID, ISO-date, title-state agreement, executable-render-script, and reciprocal arc42-link rules from Step 1. The required comment keys are `state`, `abstraction`, `question`, `owner`, `arc42`, `adrs`, and `last_verified`; the Mermaid YAML front-matter title must include the same literal `CURRENT` or `PROPOSED` state. For each diagram, resolve its declared arc42 path and require that Markdown file to contain a link whose resolved destination is the exact diagram source. Implement `validate_tooling` with standard-library JSON parsing, exact manifest/lock resolution, `git ls-files -z` tracked-path inspection, and the explicit classified-SVG marker/link/source rules from Step 1. This check remains in ordinary `CHECKS`; it is not an adoption-only assertion.
...
architecture/scripts/render-diagrams.sh
python3 architecture/scripts/validate_architecture.py --root . --checks diagrams,links,tooling
```
...
Write tests against new proposal metadata, lifecycle, bootstrap, and traceability behavior before implementation. Define the six governed proposal identities by basename: `account-identifiers-and-nip-inbound`, `conventional-deposit-products-and-accrual`, `non-interest-banking-products`, `full-poc-platform`, `production-platform`, and `providers-and-reconciliation`. Give each identity a permanent explicit HTML anchor of that basename in a `## Governed proposal registry` section of `architecture/proposals/README.md`; the immediately following single Markdown link is its mutable location pointer. Test `validate_proposal_bootstrap(root)` separately: it passes only when all six initial files and registry targets are present in the active directory and is deliberately not registered in permanent `CHECKS`. For ongoing `validate_metadata`/`validate_traceability`, require every governed identity at exactly one of `architecture/proposals/<basename>.md` or `architecture/archive/proposals/<basename>.md`, and require its registry pointer to resolve to that exact sole location; add independent missing, active-plus-archive duplicate, stale-pointer, duplicate-anchor, and wrong-basename fixtures. Active locations allow only `draft`, `proposed`, `approved`, or `implementing`. Archive locations allow only `implemented`, `rejected`, or `superseded` and require `implementation_status`, `replacement`, `implementation_evidence`, and non-empty `related_adrs`. The `related_plans` field is always required and every non-`None` entry is validated and reciprocal: `implementing` and `implemented` require one or more plans because delivery began; `rejected` and `superseded` may use literal `None` only when they never entered planning, but if they name any plan every link must be reciprocal. `implemented` requires `implementation_status: Complete` and one or more existing `architecture/arc42/*.md` replacement links; `superseded` requires `implementation_status: Not applicable` and one or more existing active/archive proposal replacement links; `rejected` requires `implementation_status: Not applicable` and literal `replacement: None`. Every terminal archive record requires at least one valid path-bound local or stable same-repository pull-request implementation/closure evidence entry and reciprocal ADR links; plan reciprocity is mandatory when plans are present, while `None` is valid only for never-planned rejected/superseded records. Add positive fixtures for implemented with plans, rejected/superseded both with reciprocal plans and with literal `None`, and focused negative fixtures for missing/invalid implementation status, replacement, evidence, ADR, required delivery plan, or present-but-nonreciprocal plan traceability.

Require allowed proposal statuses, existing paths for every non-`None` `related_plans` entry, existing `related_adrs` IDs, a `**Proposal:**` backlink to the identity's stable registry anchor in the account-identifier, conventional-deposit, and non-interest plans, reciprocal `Related proposals:` links to that same stable anchor in every ADR named by a proposal, and reciprocal stable-anchor proposal links in every plan named by a proposal. The proposal file itself retains direct ADR and any plan links; the validator resolves the registry pointer before comparing identities, so a later active-to-archive move changes only the proposal path and registry pointer, never an accepted ADR relationship entry. Reassert the generic Task 5 direct plan/ADR rule with repository fixtures for the framework, accounting-kernel, conventional-deposit, and non-interest plans: every direct ADR link in each plan has the exact ADR `Related implementation plans` backlink and every such ADR backlink has a direct plan link. Include independent missing-backlink tests for each direction. For `2026-08-30-accounting-kernel-implementation.md`, require `**Current architecture:**` links to arc42 sections 05, 06, and 08 plus `**Retrospective ADRs:**` links to ADR-0002 through ADR-0006, and explicitly reject a `**Proposal:**` backlink.

Add focused permanent infrastructure-governance fixtures for `architecture/infrastructure/infra-ubuntu24.04-poc.md`. Require front matter with `status: proposed`, an `owners` list containing only `platform`, a `related_adrs` list containing only `ADR-0008`, and a `related_proposals` list containing only the stable `architecture/proposals/README.md#full-poc-platform` identity; require the visible body marker `> **Architecture state: PROPOSED — non-current.**`. Resolve the stable registry pointer and require the full-PoC proposal record to link directly back to the infrastructure document; require ADR-0008 to link the full-PoC stable identity under `Related proposals`. Add independent failures for a current/absent state marker, wrong status/owner/ADR/proposal identity, stale registry pointer, and either missing reciprocal relationship. This document is a subordinate detail of the governed full-PoC proposal, never a seventh standalone proposal identity and never current deployment evidence.
...
Extend `validate_metadata` with the exact proposal required fields and `PROPOSAL_STATUSES = frozenset({"draft", "proposed", "approved", "implementing", "implemented", "rejected", "superseded"})`. Implement the governed-identity, unique registry-anchor/pointer, exactly-one-active-or-archive-location, active/non-terminal placement, archive/terminal placement, status-specific `implementation_status`/`replacement`, closure-evidence, always-present and status-sensitive `related_plans`, non-empty ADR link, and missing/duplicate/stale-pointer rules from Step 1. This is the permanent lifecycle contract: it requires six logical proposal records, not six permanently active paths. Implement `validate_proposal_bootstrap(root: Path) -> list[str]` as a Task 7 adoption-only assertion that all six initial records and pointers are active; do not add it to `CHECKS` or `VALIDATORS`, so later valid archival does not fail ordinary repository validation. Add a `validate_traceability(root: Path) -> list[str]` check and register `traceability`. For every proposal at either valid location, verify that each non-`None` `related_plans` path exists and contains a link to that proposal identity's stable registry anchor and that each `related_adrs` ID resolves to an ADR containing a `Related proposals:` link to the same stable anchor; resolve the pointer to prove the reciprocal link identifies the exact active/archive record. Enforce the implementing/implemented plan requirement and the never-planned rejected/superseded `None` allowance exactly. Apply the generic direct plan/ADR reciprocity from Task 5 to all governed plans, enforce the special accounting-kernel rule from Step 1, verify the three unimplemented proposal/plan mappings exactly, and permanently enforce the subordinate infrastructure metadata/state/reciprocity contract from Step 1.
...
Add proposal metadata to `architecture/infrastructure/infra-ubuntu24.04-poc.md` with `status: proposed`, an `owners` list containing only `platform`, a `related_adrs` list containing only `ADR-0008`, and a `related_proposals` list containing only the full-PoC stable registry anchor. Put the exact visible marker `> **Architecture state: PROPOSED — non-current.**` immediately after the front matter, and add a direct link to the full-PoC stable identity. Add a reciprocal direct link from the full-PoC proposal record to this infrastructure document; ADR-0008's `Related proposals` already links the stable full-PoC identity. These relationships subordinate the infrastructure detail to the governed proposal and make clear that manifests are not deployment evidence. Add `**Proposal:**` and `**Related ADRs:**` immediately below the headers of only these unimplemented plans, using these exact proposal mappings and direct ADR mappings:
...
Update every proposal-classified inventory row with the exact permanent `architecture/proposals/README.md#<governed-identity>` registry anchor and mark it `resolved`; never put an active or archive proposal-file path in the inventory. Put each exact `<!-- migration-source: SOURCE_KEY -->` marker beside its stable registry anchor, remove any temporary governance marker, and verify that the anchor's sole pointer resolves to the identity's current active record. The global marker inventory must contain no marker in a mutable proposal record and no provisional governance marker after remapping. Run:
...
Expected: metadata, links, migration inventory, lifecycle-aware bidirectional traceability, subordinate infrastructure governance, the one-time all-six-active bootstrap assertion, accepted-ADR append-only/immutability checks, and unit tests pass; every granular inventory row is now `resolved`, every final stable registry destination anchor carries the exact source-key backlink, and every proposal pointer resolves to its sole current record. Later adoption changes may move a governed proposal to its same-basename archive path and update only the mutable registry pointer with valid terminal metadata; the frozen inventory destination and review blob remain valid without running or satisfying the bootstrap-only function.
...
Use the complete reserved `00.document-preamble` plus 27-root, subsection, and material-block fixture from Task 4 rather than a path-presence-only fixture. Assert that `validate_migration_inventory` selects the old source in both pre-cutover states and the archived source in the post-cutover state, then performs the same preamble-token, granular source-key, and exact-once material-block coverage checks against the selected file. In archived fixtures, independently remove preamble token `P02` and one numbered source material block while leaving their inventory ordinals in place; each must produce a focused source/content coverage error. Neither source and both sources must each produce an error; the validator may never merge, prefer, or silently skip duplicate/missing sources.
...
Have a reviewer other than the implementer inspect the committed inventory and selected source read-only at `reviewed_commit`. The reviewer must independently confirm the reserved preamble and every numbered source block are classified exactly once, every non-historical block maps to the exact destination anchor bearing its source-key marker, every proposal destination is a stable registry anchor whose sole pointer resolves to the correct current record, every historical-only row has its required rationale, and zero rows are unresolved. Continue only when the reviewer provides their identity and literal outcome `APPROVED`; a request for changes returns execution to Task 7 and produces a new Task 7 commit before another review. The review remains bound to the immutable inventory blob; later valid proposal archival changes only a registry pointer outside that blob.
...
- Add CI-range fixtures for both PR and push ranges with exact histories: `(base) -> (introduce Proposed) -> (Accept) -> (mutate rationale)`, and `(base containing Proposed) -> (Accept) -> (mutate rationale)`. Assert `--adr-edge-base-ref <base> --adr-edge-head-ref <head>` rejects the exact accepted-to-mutated edge even when `--adr-base-ref <base> --adr-head-ref <head>` cannot detect it. Add `(base) -> (introduce Retrospective Accepted with verified pre-introduction evidence) -> (mutate rationale)` and assert the qualified introduction passes while the mutation fails; add the same history with `Retrospective: No` and assert introduction itself fails. Add a merge whose Accepted ADR differs only from its second parent and assert the range rejects that second-parent edge, proving all-parent rather than first-parent semantics.
...
- Permanent `tooling` validation parses `package.json` and `package-lock.json`, requires the direct and resolved `@mermaid-js/mermaid-cli` versions to be exactly `11.16.0`, and rejects tracked content below `architecture/tooling/node_modules/` or `architecture/diagrams/generated/`. SVGs elsewhere under `architecture/` are permitted only with the exact governed derivative marker, an existing Mermaid source, and a Markdown link; SVGs outside `architecture/` are not globally banned.
...
- Newly introduced non-retrospective ADRs must be Proposed. Direct Accepted/Rejected introduction is limited to retrospective records with verified pre-introduction historical evidence, plus the exact one-time ADR-0001 framework-adoption binding to approved design commit `0e46650dcb382bf4ddc040e0ec73e98675dff40b`; no general ADR-0001 or introduction exemption exists. Proposed ADRs may remain Proposed unchanged or with substantive, relationship/evidence, and implementation-status revisions; transition to Accepted or Rejected establishes the protected baseline. Accepted/Superseded/Deprecated protections remain permanent, and every Rejected record remains at the same path with byte-for-byte identical content on every later edge; rejected mutation, deletion, and rename are blocked.
- Every diagram's declared arc42 section contains a link back to that exact diagram source.
- Every governed proposal identity exists exactly once at its active or archive same-basename location, and its permanent registry anchor points to that sole record. Active records are non-terminal; implementing/implemented delivery has reciprocal plan history, while never-planned rejected/superseded records may use literal `related_plans: None`; every present plan link is reciprocal. Archived terminal records carry their status-specific implementation status/replacement, closure evidence, and reciprocal ADR links. The infrastructure PoC document is permanently marked `PROPOSED — non-current`, governed as a reciprocal subordinate detail of the full-PoC stable identity and ADR-0008, and is not deployment evidence. The three unimplemented plans link stable proposal identities, while the implemented accounting-kernel plan links current arc42 sections and retrospective ADRs without a proposal backlink.
...
- The granular migration inventory covers the reserved title/status/version/date/currency/audience preamble and every material heading under top-level sections `1` through `27`, has unique stable source keys, zero unresolved rows, and existing required destination anchors, exact source-key backlinks, and evidence paths. Proposal rows target stable registry anchors whose sole mutable pointers resolve to the current active/archive record, so proposal archival does not rewrite the frozen inventory or review blob.
