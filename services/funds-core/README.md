...
There are eight versioned migrations: `V001`, `V002`, `V003`, `V003.1`, `V003.2`, `V004`, additive acceptance hardening `V005`, and governed chart rotation `V006`. No application cache stores balances or journals.
...
|---|---|---|
| ACC-01 | Unbalanced and mixed-currency journals reject atomically; book-local booking/value dates, period ownership/open status, current policy, currency-qualified chart mappings and complete per-line mappings are independently guarded at commit. Partial activation, direct ACTIVE creation and post-activation mapping mutation reject; the governed atomic rotation preserves historical classification and uses one canonical chart-before-book lock order. Mapping insert/update/delete races pass five repetitions each without deadlock. The posting/close lock race passes five repeated runs. | Implemented kernel governance; live account onboarding after activation is deferred and rejected. |
